...
__device__ int32_t get_strided_idx(size_t gid, CudaVec shape, CudaVec strides, size_t offset) {
  int32_t strided_idx = offset;

  for (int i = shape.size - 1; i >= 0; --i) {
    size_t stride_idx = gid % shape.data[i];
    strided_idx += stride_idx * strides.data[i];
    gid /= shape.data[i];
  }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  out[gid] = a[get_strided_idx(gid, shape, strides, offset)];
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size,
  ...
  out[get_strided_idx(gid, shape, strides, offset)] = a[gid];
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size,
  ...
  out[get_strided_idx(gid, shape, strides, offset)] = val;
}
...
__global__ void MatmulKernel(const scalar_t* __restrict__ a,
  ...
  const int row = blockIdx.y * blockDim.y + ty;  // output row
  const int col = blockIdx.x * blockDim.x + tx;  // output col

  const bool valid_row = row < (int)M;
  const bool valid_col = col < (int)P;

  scalar_t acc = scalar_t(0);

  // All threads stay in the loop and hit both __syncthreads().
  for (int k0 = 0; k0 < (int)N; k0 += TILE) {
    const int cur = min(TILE, (int)N - k0);

    // Load A tile: fixed row, advance along K via tx
    if (valid_row && (k0 + tx) < (int)N) {
      as[ty][tx] = a[row * (int)N + (k0 + tx)];
    } else {
      as[ty][tx] = scalar_t(0);
    }

    // Load B tile: fixed col, advance along K via ty
    if (valid_col && (k0 + ty) < (int)N) {
      bs[ty][tx] = b[(k0 + ty) * (int)P + col];
    } else {
      bs[ty][tx] = scalar_t(0);
    }
    ...
    // Only do useful MACs for in-bounds output threads; others still execute the loop body
    // (safe either way) to keep control flow uniform.
  ...
  // Final write guarded by bounds
  if (valid_row && valid_col) {
    out[row * (int)P + col] = acc;
  }
}
